...
The System Architecture includes the model, the way the program is structured, and the programming languages and tools used.
...
The User Interface Design is a holistic approach to deciding how the functional elements are displayed on the screen. This includes considering the users and their experiences, as well as the necessary features and functional requirements.
...
- React Native is a popular framework for developping cross platform apps, it is compatible with windows and has a wide support base.
  - Advantages
    - Team is familiar with React Native
...
#### Xamarin
- Xamarin is a Microsoft framework for developping cross plarform apps, it is compatible with Windows and is written in C#.
  - Advantages
...
The main benefit of using existing code libraries is to save time on the project. The client would like to get the application approved by the International Chess Federation (FIDA) as soon as possible, so saving time is a priority.
...
- **react-native-confirmation-code-field**: A library for allowing user pin inputs
- **react-native-signature-capture**: A library that allows users to draw their signatures
...
It is essential to note that the user interface design process is iterative. Understanding of the user personas, features, and requirements are developed and clarified in feedback cycles with the Product Owner and other resources.
...
- Ensure tables and boards are set up and ready at each stage of a tournament.
- Make sure tournament pairings are completed and correct
...
###### What are their main concerns?
- That entering the chess moves via iPad is unfamiliar, time consuming, and distracting.
...
| ---- | ---- |
| Enter a live broadcast PGN link in Arbiter mode. | The app requires access to game data on Tornelo server for the tournament Arbiter to set up the game data collection. At MVP level, this is simply done via a QR code scan |
| Enter Pairing Selection Mode and select a board from available games in the round. | During the set up of a tournament, each device would get assigned to a separate table. The Pairing Selection Mode is a way for the tournament Arbiter to choose to which game each device is assigned. |
| Enter Table Pairing Mode. | The iPad is now used by the players who select their name and confirm their identity to start the game. |
| Enter Recording Mode, where certain actions are blocked. | Due to strict requirements of FIDE, for the app to be approved for use in official chess tournaments, the Recording Mode needs to prevent the user from taking actions (like leaving the app, going out of Game Mode, etc) |
| Enter moves, times and actions during Recording Mode. | The chess player should be able to use the app to record their chess moves, for both players, according to the owner specifications. This is a key feature of the app (as a replacement to paper scoresheets).
| Review and edit previous moves during Recording Mode. | In case the player has entered a mistake, they are able to go back into their previously entered moves and update them. |
...
- During Game Mode, it is not possible to switch to any other mode (on purpose or by accident), unless authorised by Arbiter. Only the arbiter is allowed to enter Arbiter mode during a game.
...
- It is not possible to make illegal moves, enter clock times, draw offers, and to enter a dash instead of a move.
- An automatic move counter should be available.
- The app is not allowed to correct illegal moves automatically.
...
- Users can make moves using either drag and drop or click to move methods.
...
According to the features and functional requirements, the application is broken down into the following 11 pages:
- **Splash Screen**: The splash screen is dispalyed when the app is loading
...
- **Arbiter Table Pairing Mode**: This page is displayed after the device is assigned to the table. It displays the players who will be playing at this board (in arbiter mode, the players cannot be selected). The arbiter mode allows the arbiter to return to the board selection page if needed.
- **Recording Mode: Graphic Recording**: In this mode, the user will enter moves and times using the graphical chess interface.
- **Recording Mode: Textual Recording**: In this mode, the user will enter moves and times using the textual interface.
- **Arbiter Recording Mode**: In this mode, arbiters will be able to  see why moves are illegal. If a mistake has been made, they can return to the pairing selection     mode.
- **Result Screen**: This is the screen that displays the results after a game is completed.
- **Arbiter Result Screen**: The arbiter can choose to return to recording mode if needed.
